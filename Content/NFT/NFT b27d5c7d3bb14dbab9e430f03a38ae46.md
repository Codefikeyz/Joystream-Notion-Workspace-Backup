...
This article will later be incorporated into the [**https://github.com/Joystream/handbook/blob/master/system/content-directory/broken-reference/README.md**](https://github.com/Joystream/handbook/blob/master/system/content-directory/broken-reference/README.md) document at a later time.
...
**Type:** The [**Auction Type**](https://joystream.gitbook.io/testnet-workspace/system/content-directory/nft#auction-type) representing what type of auction this is.

**Minimal Bid Step:** The minimum difference in amount between two consecutive bids in order for the new bid to be valid.

**Last Bid:** If present, is the last [**Bid**](https://joystream.gitbook.io/testnet-workspace/system/content-directory/nft#bid)which was successfully submitted.
...
![spaces-8ujeNdjza15ubRKR1XBX-uploads-git-blob-5b9406e06f42c2e2522d8e0fe6453426727c5e47-nft(2).png](NFT/spaces-8ujeNdjza15ubRKR1XBX-uploads-git-blob-5b9406e06f42c2e2522d8e0fe6453426727c5e47-nft(2).png)

NFT transactional statuses

# **NFT**

An *NFT* represents ownership title over video, and it is defined by the following information

**Owner:** The [**NFT Owner**](https://joystream.gitbook.io/testnet-workspace/system/content-directory/nft#nft-owner)representing the current owner.

**Status: The** [**Transactional Status**](https://joystream.gitbook.io/testnet-workspace/system/content-directory/nft#transactional-status) representing the state of the NFT currently.
...
The identifier value for the lock applied to root account of a new member
...
**The `ContentActor` attempting to issue the NFT.**

`video_id`

Video on which NFT is to be issued.

`royalty`

If present, the royalty for all future auctions

`metadata`

The raw metadata for the issuance.

`to`

If present, the member who should be set as the initial owner.
...
The video corresponding to the NFT for which offer is to be made.

`owner_id`

`ContentActor` identifying caller.

`to`

The beneficiary member of the offer.

`price`

If present, the amount which must be paid by beneficiary to accept offer.
...
`ContentActor` identifying the caller.

`price`

Price of buying the NFT.
...
Member attempting to buy the NFT.

`metadata`

User provided metadata associated with the purchase and ownership.
...
`ContentActor` who owns the NFT.

`video_id`

Video identifier for video to which NFT corresponds.

`auction_type`

AuctionType designating what kind of auction should be used.

`reservation_price`

Balance that sets lower bound for first valid bid amount.

`minimal_bid_step`

Balance that sets lower bound for how much each new bid must exceed last valid bid amount, if it exists.

`buy_now_price`

If provided, the Balance at which one would instantly be able to buy the NFT.

`starts_at`

If provided, the BlockNumber at which it becomes possible to bid in the auction or buy now.

`whitelist`

Set of membership identifiers which, at leats one is provided, restricts bidders or buyers to be among these.
...
To be root account of membership.
...
The video corresponding to the NFT for which sling back is to be done.
