...
It is recommended to schedule daily backups for the
...
This guide shows a simple method in order to back up
...
Suppose you want to back up orion a daily cron job at 9:00 AM CET. Provided that the
...
1. Add the following line to your crontab file. This line schedules the job to run every day at 9:00 AM CET:
...
1. Save and exit the crontab file. The cron job is now scheduled to run daily at 9:00 AM CET and will dump the PostgreSQL database to the specified backup file with the current date in the filename.